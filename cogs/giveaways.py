...
async def stop_giveaway(self, g_id, data):
    ...
    for x in winners:
        await x.send("Congratulations, you won the giveaway for {}!".format("prize"))
...
class Giveaways(commands.Cog):
    ...
    async def giveaway(self, ctx: commands.Context):
        ...
        try:
            ...
            await ctx.send(
                "You didn't mention the channel correctly, do it like {}.".format(
                    ctx.channel.mention
                )
            )
            return
    ...
    @commands.command(name="gstop", aliases=["givestop"], usage="{giveaway_id}")
    @commands.has_permissions(manage_guild=True)
